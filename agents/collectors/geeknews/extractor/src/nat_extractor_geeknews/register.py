"""NAT workflow: geeknews extractor — scrape then call validator A2A (no LLM here)."""
...
class GeekNewsExtractorConfig(FunctionBaseConfig, name="geeknews_extractor"):
    """GeekNews Extractor Agent 설정 (scrape → validator A2A 순차 호출)."""
...
async def geeknews_extractor(
    ...
    """Single-function workflow: scrape geeknews, then forward items to the validator A2A."""
    ...
    yield FunctionInfo.from_fn(
        fn=collect,
        description="Scrape GeekNews posts for an AI product and validate via the validator A2A.",
    )
