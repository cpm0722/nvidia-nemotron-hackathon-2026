"""NAT workflow: geeknews extractor — scrape, write raw, validate via validator_core, write validated.
...
class GeekNewsExtractorConfig(FunctionBaseConfig, name="geeknews_extractor"):
    """geeknews extractor 설정 (scrape → raw write → validator_core → validated write)."""
    ...
    scan_depth: int = Field(default=300, ge=0, le=2000)
    scan_workers: int = Field(default=20, ge=1, le=40)
...
async def geeknews_extractor(
    ...
    yield FunctionInfo.from_fn(
        fn=collect,
        description=(
            "Scrape geeknews for an AI product, validate via validator_core, persist "
