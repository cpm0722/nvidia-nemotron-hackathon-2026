"""NAT workflow: lobsters extractor — scrape then call validator A2A (no LLM here)."""
...
class LobstersExtractorConfig(FunctionBaseConfig, name="lobsters_extractor"):
    """Lobsters Extractor Agent 설정 (scrape → validator A2A 순차 호출)."""
...
async def lobsters_extractor(
    ...
    """Single-function workflow: scrape lobsters, then forward items to the validator A2A."""
    ...
    yield FunctionInfo.from_fn(
        fn=collect,
        description="Scrape Lobsters posts for an AI product and validate via the validator A2A.",
    )
