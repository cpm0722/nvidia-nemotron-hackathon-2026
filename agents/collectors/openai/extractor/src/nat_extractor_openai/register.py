"""NAT workflow: openai extractor — scrape then call validator A2A (no LLM here)."""
...
class OpenAIExtractorConfig(FunctionBaseConfig, name="openai_extractor"):
    """OpenAI Blog Extractor Agent 설정 (scrape → validator A2A 순차 호출)."""
...
async def openai_extractor(
    ...
    """Single-function workflow: scrape openai, then forward items to the validator A2A."""
    ...
    yield FunctionInfo.from_fn(
        fn=collect,
        description="Scrape OpenAI blog posts for an AI product and validate via the validator A2A.",
    )
