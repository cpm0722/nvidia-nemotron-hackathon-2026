"""NAT workflow: arxiv extractor — scrape, write raw, validate via validator_core, write validated.
...
class ArxivExtractorConfig(FunctionBaseConfig, name="arxiv_extractor"):
    """arxiv extractor 설정 (scrape → raw write → validator_core → validated write)."""
...
async def arxiv_extractor(
    ...
    """Scrape arxiv, persist raw + validated JSON, return validated path."""
    ...
    async def collect(raw_input: str) -> str:
        ...
        await emit_event(
            ...
            message=f"{SOURCE_NAME}: searching papers…",
        )
        ...
        await emit_event(
            ...
            message=(
                f"{SOURCE_NAME}: {scraped_count} papers scraped, validating…"
    ...
    yield FunctionInfo.from_fn(
        fn=collect,
        description=(
            "Scrape arxiv for an AI product, validate via validator_core, persist "
