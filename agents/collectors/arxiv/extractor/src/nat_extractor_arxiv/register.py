"""NAT workflow: arxiv extractor — scrape, write raw file, validate, write validated file.
...
class ArxivExtractorConfig(FunctionBaseConfig, name="arxiv_extractor"):
    """arXiv extractor 설정 (scrape → write raw → validator A2A → write validated)."""
...
async def arxiv_extractor(
    ...
    """Search arXiv, persist raw + validated JSON files, return validated path."""
    ...
    yield FunctionInfo.from_fn(
        fn=collect,
        description=(
            "Search arXiv for an AI product, validate papers via the validator A2A, "
