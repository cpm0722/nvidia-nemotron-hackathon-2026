"""NAT workflow: benchmark extractor — scrape then call validator A2A (no LLM here)."""
...
class BenchmarkExtractorConfig(FunctionBaseConfig, name="benchmark_extractor"):
    """Benchmark Extractor Agent 설정 (scrape → validator A2A 순차 호출)."""
...
async def benchmark_extractor(
    ...
    """Single-function workflow: scrape benchmark, then forward items to the validator A2A."""
    ...
    yield FunctionInfo.from_fn(
        fn=collect,
        description="Collect benchmark scores + HF discussions for an LLM and validate via validator A2A.",
    )
