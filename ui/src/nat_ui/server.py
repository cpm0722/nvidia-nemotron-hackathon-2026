...
def _load_product_payload(md_path: Path, product_name: str) -> RunProduct | None:
    """Load a single ``report_{slug}.md`` plus its ``.json`` sidecar citations."""
    try:
        markdown = md_path.read_text(encoding="utf-8").rstrip() + "\n"
    except OSError:
        return None
    citations: list[Citation] = []
    ...
    if json_path.is_file():
        try:
            ...
            data = {}
        raw = data.get("citations") if isinstance(data, dict) else None
        if isinstance(raw, list):
            for c in raw:
                ...
                citations.append(
                    Citation(
                        id=cid,
                        source=c.get("source") if isinstance(c.get("source"), str) else None,
    ...
    return RunProduct(product=product_name, markdown=markdown, citations=citations)


@app.get("/api/runs/{run_id}", response_model=RunDetail)
async def get_run(run_id: str) -> RunDetail:
    """Per-product markdown + citations for one run.

    Preferred over ``/api/reports/{run_id}`` when the frontend wants to render
    citation markers (``【N】``, ``[^N]``) as links to their sources.
    """
    ...
    ordered_products = _load_run_products(run_id)
    seen: set[Path] = set()
    products: list[RunProduct] = []

    for name in ordered_products:
        md_path = root / f"report_{_product_slug(name)}.md"
        if md_path.is_file():
            item = _load_product_payload(md_path, name)
            if item is not None:
                products.append(item)
                seen.add(md_path)

    for md_path in sorted(root.glob("report_*.md")):
        if md_path in seen:
            continue
        item = _load_product_payload(md_path, md_path.stem.removeprefix("report_"))
        if item is not None:
            products.append(item)

    if not products:
        raise HTTPException(status_code=404, detail="no report_*.md in run")
    return RunDetail(run_id=run_id, products=products)
